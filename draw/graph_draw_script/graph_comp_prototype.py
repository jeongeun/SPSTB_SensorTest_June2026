...
color_map = {
    ...
    # SPS2024 prototype FBK-LFoundry
    "FBKLF_S1": "magenta",
    "FBKLF_S2": "teal",
    "FBKLF_S3": "gray",
    "FBKLF_S4": "black",
...
# ============================================================
# SPS2024 prototype FBK-LFoundry data
# x-axis uses shifted VBias (@ -20C)
# ============================================================

data_fbk_proto_2024 = {
    "FBK-LF Sensor1 SPS2024": {
        "sensor": "FBKLF_S1",
        "setup": "sps2024",
        "full_name": "FBK-LFoundry LC1 W1/W2 Sensor 1 (TB Jun 24)",
        "BV": [100, 125, 150, 170, 180, 190, 210, 220],   # shifted (@ -20C)
        "TR": [56.70, 50.50, 43.50, 38.20, 36.90, 36.20, 33.10, 30.40],
        "CQ": [7.00, 8.10, 10.30, 13.10, 14.60, 16.90, 22.50, 27.10],
        "TR_err": None,
        "CQ_err": None,
    },

    "FBK-LF Sensor2 SPS2024": {
        "sensor": "FBKLF_S2",
        "setup": "sps2024",
        "full_name": "FBK-LFoundry LC1 W1/W2 Sensor 2 (TB Jun 24)",
        "BV": [100, 125, 150, 170, 180, 190, 200, 210, 220],   # shifted (@ -20C)
        "TR": [58.60, 51.60, 47.40, 39.00, 37.70, 35.60, 33.50, 31.50, 30.80],
        "CQ": [8.50, 9.60, 12.10, 15.30, 17.40, 20.00, 23.30, 27.80, 33.70],
        "TR_err": None,
        "CQ_err": None,
    },

    "FBK-LF Sensor3 SPS2024": {
        "sensor": "FBKLF_S3",
        "setup": "sps2024",
        "full_name": "FBK-LFoundry LC1 W1/W2 Sensor 3 (TB Jun 24)",
        "BV": [100, 125, 150, 170, 180, 190, 200, 210, 220],   # shifted (@ -20C)
        "TR": [56.60, 50.20, 46.70, 38.30, 35.00, 34.90, 33.60, 30.10, 29.50],
        "CQ": [7.60, 9.20, 11.90, 15.10, 17.00, 19.70, 22.70, 26.60, 31.80],
        "TR_err": None,
        "CQ_err": None,
    },

    "FBK-LF Sensor4 SPS2024": {
        "sensor": "FBKLF_S4",
        "setup": "sps2024",
        "full_name": "FBK-LFoundry LC1 W1/W2 Sensor 4 (TB Jun 24)",
        "BV": [100, 125, 150, 170, 180, 190, 200, 210],   # shifted (@ -20C)
        "TR": [59.20, 54.90, 45.40, 37.30, 37.50, 35.60, 33.50, 32.70],
        "CQ": [7.60, 9.00, 11.40, 14.40, 16.40, 19.10, 21.70, 25.60],
        "TR_err": None,
        "CQ_err": None,
    },
}
...
def plot_vs_bv(data, y_key, yerr_key, ylabel, output_name,
    ...
    for label, values in data.items():
        ...
        yerr = values.get(yerr_key, None)
        idx = np.argsort(bv)
        bv = bv[idx]
        y = y[idx]

        if yerr is not None:
            yerr = np.array(yerr)[idx]
...
def plot_tr_vs_cq(data, output_name,
    ...
    for label, values in data.items():
        ...
        cq_err = values.get("CQ_err", None)
        tr_err = values.get("TR_err", None)

        idx = np.argsort(cq)
        cq = cq[idx]
        tr = tr[idx]

        if cq_err is not None:
            cq_err = np.array(cq_err)[idx]
        if tr_err is not None:
            tr_err = np.array(tr_err)[idx]
...
# ============================================================
# Compare SPS2024 prototype FBK-LFoundry vs SPS2026 FBK-LF240/242
# x-axis for 2024 uses shifted BV (@ -20C)
# x-axis for 2026 uses measured BV at ~ -20.5C
# ============================================================
data_compare_fbk_2024_2026 = {}
data_compare_fbk_2024_2026.update(data_fbk_proto_2024)

fbk240_tb = data_tb_fbk["FBK-LF240 TB (SPS2026)"]
fbk242_tb = data_tb_fbk["FBK-LF242 TB (SPS2026)"]

data_compare_fbk_2024_2026["FBK-LF240 SPS2026"] = {
    "sensor": "FBK-LF240",
    "setup": "sps2026",
    "BV": fbk240_tb["BV"],
    "TR": fbk240_tb["TR"],
    "TR_err": fbk240_tb["TR_err"],
    "CQ": fbk240_tb["CQ"],
    "CQ_err": fbk240_tb["CQ_err"],
}

data_compare_fbk_2024_2026["FBK-LF242 SPS2026"] = {
    "sensor": "FBK-LF242",
    "setup": "sps2026",
    "BV": fbk242_tb["BV"],
    "TR": fbk242_tb["TR"],
    "TR_err": fbk242_tb["TR_err"],
    "CQ": fbk242_tb["CQ"],
    "CQ_err": fbk242_tb["CQ_err"],
}

# ============================================================
# Plot 7:
# Collected charge vs shifted bias voltage
# ============================================================

plot_vs_bv(
    data=data_compare_fbk_2024_2026,
    y_key="CQ",
    yerr_key="CQ_err",
    ylabel="Collected charge [fC]",
    output_name="Plot7_FBKPrototype2024_vs_FBKSPS2026_CollectedCharge_vs_ShiftedBV",
    left_text="FBK-LF prototypes SPS2024 vs FBK-LF240/242 SPS2026",
    right_text=r"T $\sim -20^\circ$C",
    xlabel=r"Bias voltage at $\sim -20^\circ$C [V]",
    ylim=(0, 40),
    legend_loc="center left",
    legend_bbox=(0.03, 0.50),
    legend_ncol=2,
    #legend_font=12,
)
...
# ============================================================
# Plot 8:
# Timing resolution vs shifted bias voltage
# ============================================================
...
plot_vs_bv(
    ...
    output_name="Plot8_FBKPrototype2024_vs_FBKSPS2026_TimingResolution_vs_ShiftedBV",
    left_text="FBK-LF prototypes SPS2024 vs FBK-LF240/242 SPS2026",
    right_text=r"T $\sim -20^\circ$C",
    xlabel=r"Bias voltage at $\sim -20^\circ$C [V]",
    ylim=(0, 70),
...
# ============================================================
# Plot 9:
# Timing resolution vs collected charge
# ============================================================

plot_tr_vs_cq(
    data=data_compare_fbk_2024_2026,
    output_name="Plot9_FBKPrototype2024_vs_FBKSPS2026_TimingResolution_vs_CollectedCharge",
    left_text="FBK-LF prototypes SPS2024 vs FBK-LF240/242 SPS2026",
    right_text=r"T $\sim -20^\circ$C",
    xlim=(0, 40),
    ylim=(0, 70),
